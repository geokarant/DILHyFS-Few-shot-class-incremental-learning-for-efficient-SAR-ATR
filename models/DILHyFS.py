...
from utils.inc_net import Cross_Attention
from models.base import BaseLearner
from utils.toolkit import target2onehot, tensor2numpy, PairedDataset
from torchprofile import profile_macs
from utils.losses import CenterLoss,FocalLoss
import sys        
import copy
...
class Learner(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        if args["early_fusion"] == "cross_attention":
            self._network = Cross_Attention(args,True)
    ...
    def replace_fc(self, trainloader, model, args):       
        ...
        with torch.no_grad():
            for i , (data_vit, data_cnn, targets) in enumerate(trainloader):
                # Ensure the labels are the same from both datasets (optional check)
                # Move data and labels to the device (GPU/CPU)
                data_vit = data_vit.to(self._device)
                data_cnn = data_cnn.to(self._device)
                label = targets.to(self._device)  # or label_cnn, since they should be the same
                embedding = model.extract_vector(data_vit, data_cnn)
    ...
    def incremental_train(self, data_manager_vit, data_manager_cnn):
        self._cur_task += 1
        self._total_classes = self._known_classes + data_manager_vit.get_task_size(self._cur_task)
        ...
        ## VIT LOAD DATASET 
        train_dataset_vit = data_manager_vit.get_dataset(np.arange(self._known_classes, self._total_classes),source="train", mode="train" )
        train_dataset_cnn = data_manager_cnn.get_dataset(np.arange(self._known_classes, self._total_classes),source="train", mode="train")
        self.train_dataset_vit = train_dataset_vit
        self.train_dataset_cnn = train_dataset_cnn
        self.data_manager_vit=data_manager_vit
        self.data_manager_cnn=data_manager_cnn

        paired_train_dataset = PairedDataset(self.train_dataset_vit, self.train_dataset_cnn)
        # Check the first batch
        self.train_loader = DataLoader(paired_train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=num_workers)
        
        test_dataset_vit = data_manager_vit.get_dataset(np.arange(0, self._total_classes), source="test", mode="test")
        test_dataset_cnn = data_manager_cnn.get_dataset(np.arange(0, self._total_classes), source="test", mode="test" )
        paired_test_dataset = PairedDataset(test_dataset_vit, test_dataset_cnn)
        self.test_loader = DataLoader(paired_test_dataset, batch_size=self.batch_size, shuffle=False, num_workers=num_workers)

        train_dataset_for_protonet_vit = data_manager_vit.get_dataset(np.arange(self._known_classes, self._total_classes), source="train", mode="test" )
        train_dataset_for_protonet_cnn = data_manager_cnn.get_dataset(np.arange(self._known_classes, self._total_classes), source="train", mode="test")
        paired_train_dataset_for_protonet = PairedDataset(train_dataset_for_protonet_vit, train_dataset_for_protonet_cnn)
        self.train_loader_for_protonet = DataLoader(paired_train_dataset_for_protonet, batch_size=self.batch_size, shuffle=False, num_workers=num_workers)
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler):
        ...
        focal_loss = FocalLoss(self.args['gamma'])
        prog_bar = tqdm(range(self.args['tuned_epoch']))
        for _, epoch in enumerate(prog_bar):
            self._network.train()
            total_loss = 0.0
            correct, total = 0, 0
            for  i, (data_vit, data_cnn, targets) in enumerate(train_loader):
                    data_vit = data_vit.to(self._device)
                    data_cnn = data_cnn.to(self._device)
                    targets = targets.to(self._device)  # or label_cnn, since they should be the same
                    
                    features = self._network.extract_vector(data_vit, data_cnn)
                    logits = self._network(data_vit,data_cnn)["logits"]
                    # Compute losses
                    if self.args['loss'] == 'ce':
                        loss = F.cross_entropy(logits, targets)
                    elif self.args['loss'] == 'focal':
                        loss = focal_loss(logits,targets)
                    elif self.args['loss'] == 'center':
                        loss = F.cross_entropy(logits, targets)
            ...
            test_acc = self._compute_accuracy_early_fusion(self._network, test_loader)
